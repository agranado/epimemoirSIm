...
divideCell2<-function(thisNode,mu,alpha,type='trit',recType="integrase",cInts=1,tInts=1,Pr_switch =1/8){
  ...
  if(recType=="integrase"){
    ...
    #epiMEMOiR recording
    for(d in 1:2){ #there are only 2 daughters, this is unlikely to change ever

        all.histories = strsplit( thisNode$epihistory ,"_")[[1]]
        ...
        for(regions in 1:length(all.histories)){
              #for each region take the correspoding values for editing and dynamic range
                  this.history = all.histories[regions]
                  ...
                  if(daughter.isopen =="1") new.chr = this.chr.acc else new.chr =this.chr.closed #this is the numeric value of chromatin rate (which is assigned after the transition)
                  #let's mutate the barcode with the new chromatin state:
                  this.barcode <- mutationScratchpad(this.barcode,mu,alpha,type,recType,cInts=0,tInts=tInts,as.numeric(new.chr))

                  all.histories[regions] = this.history #replace the array element with the updated version of the string
                  all.barcodes[regions]  = this.barcode #idem

              }    #FOR LOOP daughters ENDS
              #    Cell division
              #concatenate here:
              concat.histories = paste(all.histories,collapse="_")
              concat.barcodes =  paste(all.barcodes,collapse="_")
        #here we simulate the cell division event, after recording and mutating
        if(d==1){
          daughter$epihistory <- concat.histories
